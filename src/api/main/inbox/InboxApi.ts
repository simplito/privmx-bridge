...
export class InboxApi extends BaseApi implements inboxApi.IInboxApi {
    ...
    async inboxGet(model: inboxApi.InboxGetModel): Promise<inboxApi.InboxGetResult> {
        const cloudUser = this.sessionService.validateContextSessionAndGetCloudUser();
        const {inbox, ownGroupIds, groupEpochs} = await this.inboxService.getInbox(cloudUser, model.id, model.type);
        this.requestLogger.setContextId(inbox.contextId);
        return {inbox: this.inboxConverter.convertInbox(cloudUser.getUser(inbox.contextId), inbox, ownGroupIds, groupEpochs)};
    }
    ...
    async inboxList(model: inboxApi.InboxListModel): Promise<inboxApi.InboxListResult> {
        const cloudUser = this.sessionService.validateContextSessionAndGetCloudUser();
        const {inboxes, user, ownGroupIds, groupEpochs} = await this.inboxService.getMyInboxes(cloudUser, model.contextId, model.type, model, model.sortBy || "createDate", model.scope || "MEMBER");
        this.requestLogger.setContextId(model.contextId);
        return {inboxes: inboxes.list.map(x => this.inboxConverter.convertInbox(user.userId, x, ownGroupIds, groupEpochs)), count: inboxes.count};
    }
    
    @ApiMethod({})
    async inboxListAll(model: inboxApi.InboxListAllModel): Promise<inboxApi.InboxListAllResult> {
        const cloudUser = this.sessionService.validateContextSessionAndGetCloudUser();
        const {inboxes, user, ownGroupIds, groupEpochs} = await this.inboxService.getAllInboxes(cloudUser, model.contextId, model.type, model, model.sortBy || "createDate");
        this.requestLogger.setContextId(model.contextId);
        return {inboxes: inboxes.list.map(x => this.inboxConverter.convertInbox(user.userId, x, ownGroupIds, groupEpochs)), count: inboxes.count};
    }
