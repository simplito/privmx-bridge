...
it("elliptic import public from uncompressed buffer", () => {
    ...
    const key = EccKeyPair.fromPublic(pubBuffer);
...
it("elliptic import public from uncompressed hex", () => {
    ...
    const key = EccKeyPair.fromPublic(pubHex, "hex");
    const encoded = Buffer.from(key.getPublic().encode());
...
it("elliptic import public from compressed buffer", () => {
    ...
    const key = EccKeyPair.fromPublic(compressedPubBuffer);
    const encoded = Buffer.from(key.getPublic().encode());
...
it("elliptic import public from compressed hex", () => {
    ...
    const key = EccKeyPair.fromPublic(compressedPubHex, "hex");
    const encoded = Buffer.from(key.getPublic().encode());
...
it("elliptic get public from priv", () => {
    ...
    const key = EccKeyPair.fromPrivate(privHex, "hex");
    const encoded = Buffer.from(key.getPublic().encode());
...
it("elliptic derive", () => {
    ...
    const privKey = EccKeyPair.fromPrivate(priv);
    const pubKey = EccKeyPair.fromPublic(pub);
