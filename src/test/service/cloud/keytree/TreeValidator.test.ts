...
function assertHas(problems: TreeProblem[]|TransitionProblem[], kind: string, message?: string) {
...
describe("TreeValidator.validateRemoval", () => {
    const assignment = ["u0", "u1", "u2", "u3", "u4", "u5", "u6", "u7"];
    
    /** An honest removal: blank the leaf, refresh its direct path, advance the epoch. */
    function honestRemoval(position: number, from: string[] = assignment) {
        return treeAfterRemoval(from, position, EPOCH);
    }
    
    it("accepts an honest removal at every position", () => {
        for (let position = 0; position < assignment.length; position++) {
            const {before, after} = honestRemoval(position);
            const problems = TreeValidator.validateRemoval(before, after, user(assignment[position]), EPOCH, EPOCH + 1);
            assert.deepStrictEqual(problems, [], `position ${position}: ${JSON.stringify(problems)}`);
            // The resulting state must also be well-formed on its own terms.
            const remaining = assignment.filter((_, i) => i !== position);
            assert.deepStrictEqual(TreeValidator.validateState(after, roster(remaining), EPOCH + 1), []);
        }
    });
    
    it("accepts a removal in a truncated tree", () => {
        const from = ["a", "b", "c", "d", "e"];
        for (let position = 0; position < from.length; position++) {
            const {before, after} = honestRemoval(position, from);
            const problems = TreeValidator.validateRemoval(before, after, user(from[position]), EPOCH, EPOCH + 1);
            assert.deepStrictEqual(problems, [], `position ${position}: ${JSON.stringify(problems)}`);
        }
    });
    
    it("SECURITY: rejects a removal that leaves the leaf occupied", () => {
        const {before, after} = honestRemoval(3);
        after.leafAssignment[3] = user("u3");
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), "LEAF_NOT_BLANKED");
    });
    
    it("SECURITY: rejects a removal that skips a node on the direct path", () => {
        // Skipping even one node leaves the removed member holding a key that still decrypts current content.
        const before = buildTree(assignment, EPOCH);
        const path = TreeMath.directPath(3, assignment.length);
        const after = refresh(before, path.slice(0, path.length - 1)); // everything but the root
        after.leafAssignment[3] = "" as types.cloud.UserId;
        const problems = TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1);
        assertHas(problems, "NODE_NOT_REFRESHED");
    });
    
    it("SECURITY: rejects a bumped generation that reuses the old public key", () => {
        // The subtlest cheat available to a client: the state looks refreshed, but the private key the removed
        // member holds still matches.
        const {before, after} = honestRemoval(3);
        const rootIndex = TreeMath.root(assignment.length);
        const oldRoot = before.nodes.find(n => n.nodeIndex === rootIndex);
        const newRoot = after.nodes.find(n => n.nodeIndex === rootIndex);
        assert.ok(oldRoot && newRoot);
        newRoot.publicKey = oldRoot.publicKey;
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), "NODE_KEY_REUSED");
    });
    
    it("rejects refreshing a node off the path, which charges the group for work nobody asked for", () => {
        const {before, after} = honestRemoval(3);
        const offPath = TreeMath.copath(3, assignment.length).find(n => !TreeMath.isLeaf(n));
        assert.ok(offPath !== undefined);
        const node = after.nodes.find(n => n.nodeIndex === offPath);
        assert.ok(node);
        node.generation += 1;
        node.publicKey = `pk:${offPath}g${node.generation}` as types.core.EccPubKey;
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), "NODE_REFRESHED_NEEDLESSLY");
    });
    
    it("SECURITY: rejects a removal that does not advance the epoch", () => {
        // Without a new epoch the grant key is unchanged, every container stays readable with the old wrap, and
        // the removal accomplishes nothing beyond bookkeeping.
        const {before, after} = honestRemoval(3);
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH), "EPOCH_NOT_ADVANCED");
    });
    
    it("rejects removing somebody who holds no leaf", () => {
        const {before, after} = honestRemoval(3);
        assertHas(TreeValidator.validateRemoval(before, after, user("stranger"), EPOCH, EPOCH + 1), "MEMBER_HAS_NO_LEAF");
    });
    
    it("rejects reseating another member in the same operation", () => {
        const {before, after} = honestRemoval(3);
        after.leafAssignment[5] = user("someone-else");
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), "LEAF_ASSIGNMENT_CHANGED");
    });
    
    it("rejects changing the tree size", () => {
        const {before} = honestRemoval(3);
        const after = buildTree([...assignment, "u8"], EPOCH + 1);
        assertHas(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), "NUM_LEAVES_CHANGED");
    });
    
    it("removing the last member of a subtree is still a normal removal", () => {
        const from = ["u0", "u1", "", "u3"];
        const {before, after} = honestRemoval(3, from);
        assert.deepStrictEqual(TreeValidator.validateRemoval(before, after, user("u3"), EPOCH, EPOCH + 1), []);
    });
});

// ─────────────────────────────────────────────────────────────────────────────
// additions — and the promise that they cost the group nothing
// ─────────────────────────────────────────────────────────────────────────────

describe("TreeValidator.validateAddition", () => {
    it("accepts seating a newcomer in a blank without touching any key", () => {
        ...
        after.edges.push({
            ...
            parentGeneration: 0,
            childKind: "user",
            childUserId: user("newcomer"),
            data: "wrap:5->newcomer" as types.core.UserKeyData,
        });
        assert.deepStrictEqual(TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH), []);
        assert.deepStrictEqual(TreeValidator.validateState(after, roster(["u0", "u1", "newcomer", "u3"]), EPOCH), []);
    });
    
    it("accepts growth that creates a new root and re-links the grant edge at the same epoch", () => {
        // The whole point of the grant indirection: the tree doubles, the grant keypair does not change, and no
        // container the group can read goes stale.
        const before = buildTree(["u0", "u1"], EPOCH);
        // Three leaves, not four: the array layout truncates, so appending one member adds one leaf.
        const after = buildTree(["u0", "u1", "newcomer"], EPOCH);
        // Carry over the pre-existing subtree key so the transition looks like real growth, not a rebuild.
        const carried = after.nodes.find(n => n.nodeIndex === 1);
        const original = before.nodes.find(n => n.nodeIndex === 1);
        assert.ok(carried && original);
        carried.publicKey = original.publicKey;
        carried.generation = original.generation;
        const problems = TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH);
        assert.deepStrictEqual(problems, [], JSON.stringify(problems));
        assert.deepStrictEqual(TreeValidator.validateState(after, roster(["u0", "u1", "newcomer"]), EPOCH), []);
    });
    
    it("SECURITY: rejects an addition that advances the epoch", () => {
        // An addition that rotated the grant key would stale every container the group reads — turning a cheap
        // operation into the expensive one, and letting a single add force the whole group to re-key.
        const before = buildTree(["u0", "u1", "", "u3"], EPOCH);
        const after = clone(before);
        after.leafAssignment[2] = user("newcomer");
        const grant = after.edges.find(e => e.isGrantEdge);
        assert.ok(grant);
        grant.parentGeneration = EPOCH + 1;
    ...
    it("rejects seating over an occupied leaf", () => {
        const before = buildTree(["u0", "u1", "u2", "u3"], EPOCH);
        const after = clone(before);
        after.leafAssignment[2] = user("newcomer");
        const problems = TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH);
        assertHas(problems, "SEAT_NOT_BLANK_BEFORE");
    });
    
    it("rejects a seat the newcomer did not actually take", () => {
        const before = buildTree(["u0", "u1", "", "u3"], EPOCH);
        const after = clone(before);
        assertHas(TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH), "SEAT_NOT_TAKEN");
    });
    
    it("rejects a tree size that does not match the position being seated", () => {
        const before = buildTree(["u0", "u1"], EPOCH);
        const after = buildTree(["u0", "u1", "newcomer", "", "", "", "", ""], EPOCH);
        assertHas(TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH), "NUM_LEAVES_WRONG_FOR_POSITION");
    });
    
    it("rejects moving an existing member while adding", () => {
        const before = buildTree(["u0", "u1", "", "u3"], EPOCH);
        const after = clone(before);
        after.leafAssignment[2] = user("newcomer");
        after.leafAssignment[1] = "" as types.cloud.UserId;
        after.leafAssignment[0] = user("u1");
        const problems = TreeValidator.validateAddition(before, after, user("newcomer"), 2, EPOCH, EPOCH);
        assertHas(problems, "LEAF_ASSIGNMENT_CHANGED");
    });
});
